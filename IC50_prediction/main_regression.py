...
def train_test(drug_model_params, train_loader, test_loader, arg, celllinefeature, train_rna,
    ...
    for epoch in range(arg.epochs):
        ...
        mse = mean_squared_error(final_y, final_output)
        rmse = np.sqrt(mse)
        mae = mean_absolute_error(final_y, final_output)
        r2 = r2_score(final_y, final_output)
        evs = explained_variance_score(final_y, final_output)

        pcc, p_value = pearsonr(final_y, final_output)
        scc, s_p_value = spearmanr(final_y, final_output)
        conindex = concordance_index(final_y, final_output)

        print(f'epoch: {epoch}')
        print(
            f'MSE: {mse:.4f}; RMSE: {rmse:.4f}; MAE: {mae:.4f}; R-squared: {r2:.4f}; Explained Variance Score: {evs:.4f};'
            f' PCC: {pcc:.4f}; SCC: {scc:.4f}; p_value: {p_value}; s_p_value: {s_p_value}; concordance_index: {conindex: .4f}')

        if pcc > best_pcc and scc > best_scc:
            best_pcc, best_scc = pcc, scc
            best_model = copy.deepcopy(model)
            result = {
                "MSE": mse,
                "RMSE": rmse,
...
def main(arg, drug_model_params):
    print(f'drug_model_params: {drug_model_params}')
    with open(arg.root_path + "IC50.pkl", "rb") as f:
        IC50 = pickle.load(f)
    ...
    mlp_dim = celllinefeature.shape[1]

    print(f'celllinefeature: {celllinefeature.shape}')
